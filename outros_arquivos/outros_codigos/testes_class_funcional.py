...
class SentenceEncoder:
    ...
    def _apply_pooling(self, output, attention_mask):
        ...
        strategies = {
            "cls": lambda: output.last_hidden_state[:, 0, :],
            "mean": lambda: self._mean_pooling(output, attention_mask),
            ...
            "sum_all_layers": lambda: torch.stack(hidden_states).sum(0).mean(1),
            ...
            "sum_all_layers_cls": lambda: torch.stack(hidden_states).sum(0)[:, 0, :],
...
def main():
    ...
    parser.add_argument("--pooling", type=str, default="cls,mean,mean_no_cls_sep,mean_4_layers,sum_all_layers,mean_cls_mean_tokens,concat_cls_mean_tokens,mean_4_layers_cls,sum_all_layers_cls,mean_cls_mean_4_layers,concat_cls_mean_4_layers,concat_3_means,concat_5_means", help="Pooling strategies")
    ...
    for model_name in models:
        ...
        results = run_senteval(model_name, pooling_strategies, classification_tasks, args.task_path, args.epochs)
        for pooling, res in results.items():
            classification_results = [res.get(task, {}).get('acc', 0) for task in classification_tasks]
            #similarity_results = [res.get(task, {}).get('spearman', 0) for task in similarity_tasks]
            ...
            #df2.to_csv(f'similarity_results_' + models[0].replace('/', '_')+ '_intermediate.csv', index=False)
    ...
    #similarity_df.to_csv(f'similarity_results_' + models[0].replace('/', '_')+ '.csv', index=False)

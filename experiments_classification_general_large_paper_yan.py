...
def main():
    ...
    parser.add_argument("--models", type=str, required=True, default="bert-large-uncased,roberta-large", help="Modelos HuggingFace separados por vírgulas")
    ...
    # Configurar o logger
    logging.basicConfig(
        filename=f'logcl' + "_nhid-" + str(args.nhid_number) + '_' + '_'.join([st.replace('/', '_') for st in models]) + "_epochs-" + str(args.epochs) + '.txt',  # Nome do arquivo de saída
    ...
    for model_name in models:
        ...
        df1.to_csv(f'cl' + "_nhid-" + str(args.nhid_number) + '_' + '_'.join([st.replace('/', '_') for st in models]) + "_epochs-" + str(args.epochs) + '_intermediate.csv', index=False)
            
    # Salvar os resultados finais como DataFrame
    classification_df = pd.DataFrame(classification_results_data)
    classification_df.to_csv(f'cl' + "_nhid-" + str(args.nhid_number) + '_' + '_'.join([st.replace('/', '_') for st in models]) + "_epochs-" + str(args.epochs) + '.csv', index=False)
